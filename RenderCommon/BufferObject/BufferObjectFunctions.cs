...
{
    public static Rect GetBoundingBox(IEnumerable<Point2D> points) 
        => Rect.FromBounds(points.Min(p => p.X), points.Min(p => p.Y), points.Max(p => p.X), points.Max(p => p.Y));

    public static Rect GetBoundingBox(params Point2D[] points) => GetBoundingBox(points);

    public static Rect GetBoundingBox(IEnumerable<IBoundingBox> boundingBoxes)
        => Rect.FromBounds(
    ...
    public static Rect GetBoundingBox(params IBoundingBox[] boundingBoxes) => GetBoundingBox(boundingBoxes);
}
