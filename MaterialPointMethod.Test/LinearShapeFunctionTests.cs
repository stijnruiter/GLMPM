...
[TestFixture]
internal class LinearShapeFunctionTests
{
    ...
    public void SampleSimpleShapeFunction()
    {
        ...
        var shapeFunction = new LinearShapeFunction(nodes);
    ...
    public void SampleSimpleShapeFunctionException()
    {
        var nodes = new float[] { 0f, 1f, 2f, 3f, 4f };
        var shapeFunction = new LinearShapeFunction(nodes);
        Assert.Throws<ArgumentOutOfRangeException>(() => shapeFunction.Sample(-1f, 1));
        Assert.Throws<ArgumentOutOfRangeException>(() => shapeFunction.Sample(4.1f, 1));
        Assert.Throws<IndexOutOfRangeException>(() => shapeFunction.Sample(3.0f, -1));
        Assert.Throws<IndexOutOfRangeException>(() => shapeFunction.Sample(3.0f, 5));
    }
}
